...
using FinEtoolsFlexStructures.VisUtilModule: plot_nodes, plot_midline, render, plot_space_box, plot_solid, space_aspectratio, default_layout_3d
using PlotlyJS
using JSON
...
function curve_mesh()
    ...
    plots = cat(plot_nodes(fens),
        plot_midline(fens, fes; color = "rgb(155, 15, 255)", lwidth = 8),
        dims = 1)
    # push!(plots, plot_nodes(fens))
    layout = default_layout_3d(;autosize = true)
    layout[:scene][:aspectmode] = "data"
    pl = render(plots; layout=layout, title = "Mesh")
