...
function _execute(n = 8, visualize = true)
    ...
    femm.drilling_stiffness_scale = 0.0
    femm.threshold_angle = 90.0
    ...
    es = eigen(Matrix(K))
    @show es.values
    @show nzv = length([zv for zv in es.values if abs(zv) < 1.0e-1])
    vectors = []
    for k in 1:nzv+6
        scattersysvec!(dchi, es.vectors[:, k])
        push!(vectors, ("mode_displacements_$(k)_$(es.values[k])", deepcopy(dchi.values[:, 1:3])))
        push!(vectors, ("mode_rotations_$(k)_$(es.values[k])", deepcopy(dchi.values[:, 4:6])))
    end
    vtkwrite("hemisphere_open-$n-modes.vtu", fens, fes; vectors = vectors)
    # U = K\F
