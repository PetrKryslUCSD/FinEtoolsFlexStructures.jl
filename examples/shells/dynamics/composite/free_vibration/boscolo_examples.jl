...
function _execute_t3ff(aspect, n, refndom, visualize)
...
function _execute_q4rs(aspect, n, refndom, visualize)
    ...
    vtkwrite("boscolo_q4rs_n=$n.vtu", fens, fes)
...
function test_convergence()
    ...
    for (aspect, refndom) in zip([2, 4, 5, 10, 20, 25], refndoms)
        for n in [25]
            _execute_t3ff(aspect, n, refndom, false)
        end
    end
    @info "Boscolo-Banerjee plate vibration"
    @info "Reference from Table 3, DySAP"
    ...
    for (aspect, refndom) in zip([2, 4, 5, 10, 20, 25], refndoms)
        for n in [25]
            _execute_q4rs(aspect, n, refndom, false)
        end
    end
    return true
end
