...
Reference values: ? [Hz] Inﬂuence of Shear Correction Factors on
Eigenfrequencies of Layered Plates, Friedrich Gruttmann and Werner Wagner
Article in H. Altenbach et al. (eds.), Advances in Mechanics of Materials and
...
Table 2 for the [30, -30, -30, 30] layup.
...
function _execute(n, reference, visualize)
    ...
    layup = CM.CompositeLayup("gruttman_$nplies", plies, mcsys)
...
function test_convergence()
    @info "Symmetric angle-ply plate vibration"
    @info "Angle: 30"
    @info "Number of plies: 4"
    @info "Reference from Gruttmann, Wagner"
    reference = (
    253.20, 444.31, 669.01, 710.02, 861.31, 1020.29, 1148.31, 1187.15, 1352.61, 1352.36
    )
    ns = [10, 20, 40, 80, 160]
    for n in ns
            _execute(n, reference, true)
        end
