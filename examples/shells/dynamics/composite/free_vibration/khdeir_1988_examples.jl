...
function _execute_ss_t3ff(nplies, angle, n, reference, visualize)
    ...
    if visualize
        ...
        vtkwrite("khdeir_1988_t3ff-SS-a=$angle-np=$(nplies)-modes.vtu", fens, fes; vectors=vectors)
            # update_rotation_field!(Rfield0, dchi)
...
function _execute_ss_q4rs(nplies, angle, n, reference, visualize)
    ...
    if visualize
        ...
        vtkwrite("khdeir_1988_q4rs-SS-a=$angle-np=$(nplies)-modes.vtu", fens, fes; vectors=vectors)
            # update_rotation_field!(Rfield0, dchi)
            # plots = cat(plot_space_box([[0 0 -L/2]; [L/2 L/2 L/2]]),
            #     plot_nodes(fens),
            #     plot_midsurface(fens, fes; x = geom0.values, u = dchi.values[:, 1:3], R = Rfield0.values);
...
function test_ss()
    ...
    for nplies in [2, 4, 10]
        ...
        for angle in [30, 45, ]
            @info "Angle: $angle"
            for n in [10, 20, 40, ]
                _execute_ss_t3ff(nplies, angle, n, reference["$nplies/$angle"], true)
            end
        end
    end
    @info "\nTesting Q4RS"
    for nplies in [2, 4, 10]
        ...
        for angle in [30, 45, ]
            @info "Angle: $angle"
            for n in [10, 20, 40, ]
                _execute_ss_q4rs(nplies, angle, n, reference["$nplies/$angle"], true)
            end
        end
    end
...
function _execute_ff_t3ff(nplies, angle, n, reference, visualize)
    formul = FEMMShellT3FFCompModule
    CM = CompositeLayupModule
    a = 1.0 * phun("m")
    thickness = a / 10
    ...
    if visualize
        ...
        vtkwrite("khdeir_1988_t3ff-FF-a=$angle-np=$(nplies)-n=$n-modes.vtu", fens, fes; vectors=vectors)
            # update_rotation_field!(Rfield0, dchi)
            # plots = cat(plot_space_box([[0 0 -L/2]; [L/2 L/2 L/2]]),
            #     plot_nodes(fens),
            #     plot_midsurface(fens, fes; x = geom0.values, u = dchi.values[:, 1:3], R = Rfield0.values);
...
function _execute_ff_q4rs(nplies, angle, n, reference, visualize)
    formul = FEMMShellQ4RSCompModule
    CM = CompositeLayupModule
    a = 1.0 * phun("m")
    thickness = a / 10
    ...
    if visualize
        ...
        vtkwrite("khdeir_1988_q4rs-FF-a=$angle-np=$(nplies)-n=$n-modes.vtu", fens, fes; vectors=vectors)
            # update_rotation_field!(Rfield0, dchi)
            # plots = cat(plot_space_box([[0 0 -L/2]; [L/2 L/2 L/2]]),
            #     plot_nodes(fens),
            #     plot_midsurface(fens, fes; x = geom0.values, u = dchi.values[:, 1:3], R = Rfield0.values);
...
function test_ff()
    ...
    for nplies in [2, 4, 10]
        ...
        for angle in [30, 45, ]
            @info "Angle: $angle"
            for n in [10, 20, 40, ]
                _execute_ff_t3ff(nplies, angle, n, reference["$nplies/$angle"], true)
            end
        end
    end
    @info "\nTesting Q4RS"
    for nplies in [2, 4, 10]
        ...
        for angle in [30, 45, ]
            @info "Angle: $angle"
            for n in [10, 20, 40, ]
                _execute_ff_q4rs(nplies, angle, n, reference["$nplies/$angle"], true)
            end
        end
    end
