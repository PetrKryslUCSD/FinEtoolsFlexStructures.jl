...
From Table 6, nondimensional frequencies, analytical
23, 45, 71, 80, 91, 126, 129
"""
...
const C11 = 159.85*phun("GPa")
const REFERENCE_NORMALIZED_FREQUENCIES = [
    23, 45, 71, 80, 91, 126, 129
]

function _execute_t3ff(n, visualize)
    ...
    fs = oms ./(2*pi)
    @info "Frequencies: $(round.(fs, digits=2)) [Hz]"
    reffs = REFERENCE_NORMALIZED_FREQUENCIES 
    @info "Reference frequencies: $(reffs) [Hz]"
...
function _execute_q4rs(n, visualize)
    ...
    vtkwrite("nayak_sandwich_q4rs_n=$n.vtu", fens, fes)
...
function test_convergence()
    @info "Nayak et al sandwich plate vibration, T3FF"
    for n in [10, 100]
        _execute_t3ff(n, true)
    end
    @info "Nayak et al sandwich plate vibration, Q4RS"
    for n in [10, 100]
