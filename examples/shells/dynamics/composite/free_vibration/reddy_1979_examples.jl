...
function _execute_t3ff(mid, angle, n, reference, visualize)
    ...
    @info "Nondim. freq. ($bestfit): $(round(ndoms[bestfit], digits=4))  vs ref: $(round(reference, digits=4)): $(round(ndoms[bestfit]/reference*100, digits=2)) %"
...
function _execute_q4rs(mid, angle, n, reference, visualize)
...
function test()
    ...
    for mid in [1, 2]
        ...
        for angle in [0, 30, 45, ]
            ...
            for n in [10, 20, 40, 80]
                _execute_t3ff(mid, angle, n, reference["$mid/$angle"], true)
            end
        end
    end
    ...
    for mid in [1, 2]
        ...
        for angle in [0, 30, 45, ]
            @info "Angle: $angle"
            for n in [10, 20, 40, 80]
                _execute_q4rs(mid, angle, n, reference["$mid/$angle"], true)
            end
        end
    end
