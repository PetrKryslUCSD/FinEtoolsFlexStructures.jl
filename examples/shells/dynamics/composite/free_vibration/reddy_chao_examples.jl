...
function _execute_t3ff(aspect, n, reference, visualize)
    ...
    @info "Nondim freq: $(round(ndom, digits=4)) ($(round(ndom/reference*100, digits=2)) %)"
...
function _execute_q4rs(aspect, n, reference, visualize)
...
function test_convergence()
    ...
    for aspect in [5, 10, 25, 50]
        for n in [10, 20, 40]
            _execute_t3ff(aspect, n, reference["a/h=$aspect"], false)
        end
    end
    @info "Testing Q4RS elements"
    for aspect in [5, 10, 25, 50]
        for n in [10, 20, 40]
            _execute_q4rs(aspect, n, reference["a/h=$aspect"], false)
        end
    end
    return true
end
