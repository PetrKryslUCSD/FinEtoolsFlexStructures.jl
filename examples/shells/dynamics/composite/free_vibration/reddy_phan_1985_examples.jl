...
(Either two or eight layers.)

Aspect ratio variable, 5, 10, 20, 50.
...
function _execute_angle_2_or_8_t3ff(nplies, aspect, n, reference, visualize)
    ...
    @info "Frequency: $(round(fs[1]; sigdigits=4)). Nondim freq: $(round(ndoms[1]; sigdigits=4))  vs Reference: $(round(reference; sigdigits=4)) ($(round(ndoms[1]/reference*100; sigdigits=4)))"
...
function _execute_angle_2_or_8_q4rs(nplies, aspect, n, reference, visualize)
...
function _execute_angle_1_or_3(nplies, aspect, n, reference, visualize)
    formul = FEMMShellT3FFCompModule
    CM = CompositeLayupModule
    angle = 45
    a = 1.0 * phun("m")
    ...
    fs = oms ./(2*pi)
    ndoms = nondimensionalised_frequency(material, a, thickness, oms)
    @info "Frequency: $(round(fs[1]; sigdigits=4)). Nondim freq: $(round(ndoms[1]; sigdigits=4)) "
...
function test_angle_2_or_8()
    ...
    @info "========================================="
    @info "Testing T3FF elements"
    for nplies in [2, 8]
        @info "-----------------------------------------\nNumber of plies: $nplies"
        for aspect in [5, 10, 20, 50, 100]
            @info "Aspect: $aspect"
            for n in [10, 20, 40,]
                _execute_angle_2_or_8_t3ff(nplies, aspect, n, reference["nplies=$nplies, a/h=$aspect"], true)
            end
        end
    end
    @info "========================================="
    @info "Testing Q4RS elements"
    for nplies in [2, 8]
        @info "-----------------------------------------\nNumber of plies: $nplies"
        for aspect in [5, 10, 20, 50, 100]
            @info "Aspect: $aspect"
            for n in [10, 20, 40,]
                _execute_angle_2_or_8_q4rs(nplies, aspect, n, reference["nplies=$nplies, a/h=$aspect"], true)
            end
        end
    end
    return true
...
function test_angle_1_or_3()
    @info "Antisymmetric angle-ply laminated plate vibration"
    @info "Reference from Reddy, Phan 1985.  "
    reference = Dict(
        "nplies=1, a/h=5" => 10.335,
        "nplies=1, a/h=10" => 13.044,
        "nplies=1, a/h=20" => 14.179,
        "nplies=1, a/h=50" => 14.561,
        "nplies=1, a/h=100" => 14.618,
        "nplies=3, a/h=5" => 12.892,
        "nplies=3, a/h=10" => 19.289, 
        "nplies=3, a/h=20" => 23.259,
        "nplies=3, a/h=50" => 24.909,
        "nplies=3, a/h=100" => 25.176
    )
    for nplies in [1, 3]
        @info "-----------------------------------------\nNumber of plies: $nplies"
        for aspect in [5, 10, 20, 50, 100]
            @info "Aspect: $aspect"
            for n in [10, 20, 40,]
                _execute_angle_1_or_3(nplies, aspect, n, reference["nplies=$nplies, a/h=$aspect"], true)
            end
        end
    end
...
function allrun()
    println("#####################################################")
    println("# test_angle_2_or_8 ")
    test_angle_2_or_8()
    # println("#####################################################")
    # println("# test_angle_1_or_3 ")
    # test_angle_1_or_3()
    return true
end # function allrun
