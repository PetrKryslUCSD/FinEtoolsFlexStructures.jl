...
"""
module plate_expl_damp_explore_examples
...
function cd_loop!(M, K, ksi, U0, V0, tend, dt, force!, peek)
    U = similar(U0)
    V = similar(U0)
    A = similar(U0)
    F = similar(U0)
...
function predcorr_loop!(M, K, ksi, U0, V0, tend, dt, force!, peek)
    U = similar(U0)
    V = similar(U0)
    A = similar(U0)
    F = similar(U0)
    ...
    for step in 1:nsteps
        t = t + dt
        @. U += dt*V + dt2_2*A; # Displacement update
        @. V += dt_2 * A # Predict the velocity
        force!(F, t); # External loading
        F .-= ThreadedSparseCSR.bmul!(E, K, U) # Add elastic restoring forces
        @. F -= C * V # Add damping force
        @. A = invM * F # Compute the acceleration
        @. V += dt_2 * A # Correct the velocity
        peek(step, U, V, t)
    end
end
...
function _execute(nref = 2, nthr = 0)
    ...
    if algorithm == :cd
        cd_loop!(M, K, ksi, U0, V0, nsteps*dt, dt, force!, peek)
    else
        predcorr_loop!(M, K, ksi, U0, V0, nsteps*dt, dt, force!, peek)
    end
