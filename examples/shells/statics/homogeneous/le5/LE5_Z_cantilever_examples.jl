...
function _execute_model_t3ff(input = "nle5xf3c.inp", nrefs = 0, visualize = true)
...
function _execute_model_q4rs(input = "nle5-q4.inp", nrefs = 0, visualize = false)
    ...
    sfes = FESetShellQ4()
    accepttodelegate(fes, sfes)
    femm = formul.make(IntegDomain(fes, Simpson13Rule(2), thickness), ocsys, mater)
    stiffness = formul.stiffness
    associategeometry! = formul.associategeometry!
    ...
    # function inspector(idat, j, conn, x, out, loc)
    ...
    # inspectintegpoints(femm, geom0, dchi, [3], inspector, [], :membrane, outputcsys = ocsys)
...
function _execute_model_t3ff_alt(ignore = "", nrefs = 0, visualize = true)
    formul = FEMMShellT3FFModule
    E = 210e9;
    nu = 0.3;
    L = 10.0;
...
function test_convergence_t3ff()
    ...
    for n in [0, 1, 2, 3, 4, ]
        _execute_model_t3ff("nle5xf3c.inp", n, false)
    end
    return true
end

function test_convergence_t3ff_alt()
    formul = FEMMShellT3FFModule
    @info "LE5 Z-cantilever, formulation=$(formul)"
    for n in [0, 1, 2, 3, 4, 5]
        _execute_model_t3ff_alt("ignore", n, false)
    end
    return true
end
...
function test_convergence_q4rs()
    ...
    @info "LE5 Z-cantilever, formulation=$(formul)"
    for n in [0, 1, 2, 3, 4, ]
        _execute_model_q4rs("nle5-q4.inp", n, false)
    end
    return true
end
...
function allrun()
    ...
    println("#####################################################")
    println("# test_convergence_t3ff_alt ")
    test_convergence_t3ff_alt()
    return true
end # function allrun
