...
function _execute_t3ff(n = 8, visualize = true)
    E = 1.0;
    nu = 1.0/3;
    width = 48.0; 
    height = 44.0; 
...
function _execute_q4rtn(n = 8, visualize = true)
    E = 1.0;
    nu = 1.0/3;
    width = 48.0; 
    height = 44.0; 
    ...
    # Suppress drilling: TODO remove to test
    ...
    if !visualize
        ...
        # pl = render(plots)
        # update_rotation_field!(Rfield0, dchi)
        vtkwrite("cook_membrane-n=$n-uur.vtu", fens, fes; vectors = [("u", dchi.values[:, 1:3]), ("ur", dchi.values[:, 4:6])])
...
function test_convergence()
    ...
    @info "Cook trapezoidal membrane T3FF"
