...
function _execute_q4rs_model(
    ...
    elseif mesh == :graded
        xs = gradedspace(0.0, L/2, n+1, 3)
        ys = gradedspace(0.0, L/2, n+1, 3)
        fens, fes = Q4blockx(xs, ys);
        minimum(abs.(diff(xs))) / 3
    elseif mesh == :striped
        xs = L/2 .* vcat(linearspace(0.0, tL_ratio, Int(n/2)), linearspace(tL_ratio, 1.0, Int(n/2))[2:end])
...
function plot()
    ...
    for edge in ["hori", "vert"]
        for nc in ncs
            ...
            display(ax)
            pgfsave("pl-nu=$nu-skew=$skew-$(edge)-$(res)$(nc).pdf", ax)
        end
    end
    return true
end
