...
function _execute_dual(mesh_procedure = :q4_t3, n = 2, t_radius_ratio = 0.01, visualize = true)
    E = 200*phun("GPa");
    nu = 0.3;
    a = 1.0*phun("m");
    t = a * t_radius_ratio;
    ...
    # analytical solution for the vertical deflection under the load
    # Includes the effect of shear deformation.
    analyt_sol = -q*a^4/64/D*((5+nu)/(1+nu)+4/(1-nu)*t^2/a^2);
    formul = FEMMShellT3FFModule
    thickness = (loc, conn, ip) -> ForwardDiff.Dual(t);
    ...
    sfes = FESetShellT3()
    accepttodelegate(fes, sfes)
    femm = formul.make(IntegDomain(fes, TriRule(1), thickness, false), CSys(3, 3, ForwardDiff.Dual(0.0), formul.isoparametric!), mater)
    stiffness = formul.stiffness
    associategeometry! = formul.associategeometry!
    ...
    # Assemble the system matrix
    associategeometry!(femm, geom0)
    K = stiffness(femm, SysmatAssemblerSparse(ForwardDiff.Dual(0.0)), geom0, u0, Rfield0, dchi);
    @info "Done K"
    # Load
    nl = selectnode(fens; box = Float64[0 0 0 0 -Inf Inf], tolerance = tolerance)
    lfemm = FEMMBase(IntegDomain(fes, TriRule(3)))
...
function test_dual()
    ...
    @info "thickness/length = $t_radius_ratio "
    for n in [2, 4, 8, 16, 32, 64]
        _execute_dual(:t3_poor, n, t_radius_ratio, false)
    end
    return true
end
