...
system vectors for linear layered (composite) shells using the quadrilateral
...
"""
module FEMMShellQ4RSCompModule
...
using FinEtools.MatrixUtilityModule:
    add_btdb_ut_only!, complete_lt!, locjac!, add_nnt_ut_only!, add_btsigma!, add_b1tdb2!
...
using ..CompositeLayupModule:
    ...
    laminate_transverse_stiffness!,
    plane_stress_Tbar_matrix!,
    transverse_shear_T_matrix!,
    laminate_inertia!
...
"""
    mutable struct FEMMShellQ4RSComp{ID<:IntegDomain{S} where {S<:FESetQ4}, T<:Real, F<:Function, CS<:CSys{T}, M} <: AbstractFEMM
...
For details about the homogeneous-shell refer to [`FinEtoolsFlexStructures.FEMMShellQ4RSModule.FEMMShellQ4RS`](@ref).
"""
mutable struct FEMMShellQ4RSComp{ID<:IntegDomain{S} where {S<:FESetQ4}, T<:Real, F<:Function} <: AbstractFEMM
...
_number_type(femm::FEMMShellQ4RSComp{ID, T, CS}) where {ID, T, CS} = T
...
"""
    ...
    ) where {ID<:IntegDomain{S} where {S<:FESetQ4}, CL<:AbstractCompositeLayup, F<:Function}
...
```
All elements use a single layup.
"""
function FEMMShellQ4RSComp(
    integdomain::ID,
    layup::CL,
    stab_fun::F = _stab_fun,
) where {ID<:IntegDomain{S} where {S<:FESetQ4}, CL<:AbstractCompositeLayup, F<:Function}
    T = typeof(layup.offset)
    _nnmax = 0
...
Make a Q4RS FEMM from the integration domain,  and a layup.
...
function make(integdomain, layup, stab_fun::F = _stab_fun) where {F<:Function}
    return FEMMShellQ4RSComp(integdomain, layup, stab_fun)
end
...
"""
    associategeometry!(self::FEMMShellQ4RSComp,  geom0::NodalField{FFlt})

Associate geometry with the FEMM. 

In this case it means evaluate the nodal normals.
"""
function associategeometry!(self::FEMMShellQ4RSComp, geom::NodalField{FFlt})
    ...
    for lg in self.layup_groups
        ...
        eset = lg[2]
        for el in eset
            gathervalues_asmat!(geom, ecoords, fes.conn[el])
            for j in 1:__NN
                locjac!(loc, J, ecoords, Ns[j], gradNparams[j])
                Jac = Jacobianmdim(self.integdomain, J, loc, fes.conn[el], Ns[j], 2)
                _compute_nodal_normal!(nnormal, layup.csys, loc, J, el, j)
...
"""
    num_normals(self::FEMMShellQ4RSComp)

Compute the summary of the nodal normals.
"""
function num_normals(self::FEMMShellQ4RSComp)
    normals, normal_valid = self._normals, self._normal_valid
...
"""
    stiffness(self::FEMMShellQ4RSComp, assembler::ASS, geom0::NodalField{FFlt}, u1::NodalField{TI}, Rfield1::NodalField{TI}, dchi::NodalField{TI}) where {ASS<:AbstractSysmatAssembler, TI<:Number}

Compute the material stiffness matrix.
"""
function stiffness(
    self::FEMMShellQ4RSComp,
    assembler::ASS,
    ...
    for lg in self.layup_groups
        ...
        for i in eset # Loop over elements in the layup group
            ...
            for j in 1:npts
                ...
                tts!(sH, Tts)
                bmmat!(Bm, gradN_e, T)
                add_btdb_ut_only!(elmat, Bm, Jac * w[j], sA, DpsBmb)
                bbmat!(Bb, gradN_e, T)
                add_btdb_ut_only!(elmat, Bb, Jac * w[j], sD, DpsBmb)
                add_b1tdb2!(elmat, Bm, Bb, Jac * w[j], sB, DpsBmb)
                add_b1tdb2!(elmat, Bb, Bm, Jac * w[j], sB, DpsBmb)
                bsmat!(Bs, ecoords_e, pc[j, :], T)
                add_btdb_ut_only!(elmat, Bs,
                    stab_fun(t, h) * Jac * w[j],
                    sH, DtBs)
            end
...
"""
    mass(self::FEMMShellQ4RSComp,  assembler::A,  geom::NodalField{FFlt}, dchi::NodalField{TI}) where {A<:AbstractSysmatAssembler, TI<:Number}
