...
"""
    FEMMShellT3FFA{S<:AbstractFESet, F<:Function} <: AbstractFEMM
...
"""
mutable struct FEMMShellT3FFA{S<:AbstractFESet, F<:Function, M} <: AbstractFEMM
...
function FEMMShellT3FFA(integdomain::IntegDomain{S, F}, mcsys::CSys, material::M) where {S<:AbstractFESet, F<:Function, M}
    _nnmax = 0
    ...
    return FEMMShellT3FFA(integdomain, mcsys, 
...
function FEMMShellT3FFA(integdomain::IntegDomain{S, F}, material::M) where {S<:AbstractFESet, F<:Function, M}
    return FEMMShellT3FFA(integdomain, CSys(3, 3, isoparametric!), material)
end

function make(integdomain, material)
    return FEMMShellT3FFA(integdomain, material)
end

function make(integdomain, mcsys, material)
    return FEMMShellT3FFA(integdomain, mcsys, material)
end
...
function associategeometry!(self::FEMMShellT3FFA,  geom::NodalField{FFlt})
...
function num_normals(self::FEMMShellT3FFA)
    normals, normal_valid = self._normals, self._normal_valid
...
"""
    stiffness(self::FEMMShellT3FFA, assembler::ASS, geom0::NodalField{FFlt}, u1::NodalField{TI}, Rfield1::NodalField{TI}, dchi::NodalField{TI}) where {ASS<:AbstractSysmatAssembler, TI<:Number}

Compute the material stiffness matrix.
"""
function stiffness(self::FEMMShellT3FFA, assembler::ASS, geom0::NodalField{FFlt}, u1::NodalField{TI}, Rfield1::NodalField{TI}, dchi::NodalField{TI}) where {ASS<:AbstractSysmatAssembler, TI<:Number}
    ...
    for i in 1:count(fes) # Loop over elements
        ...
        he = sqrt(2*Ae)
        _Bsmat1!(Bs, ecoords_e)
        add_btdb_ut_only!(elmat, Bs, (1/3)*(t^3/(t^2+mult_el_size*he^2))*Ae, Dt, DtBs)
        _Bsmat2!(Bs, ecoords_e)
        add_btdb_ut_only!(elmat, Bs, (1/3)*(t^3/(t^2+mult_el_size*he^2))*Ae, Dt, DtBs)
        _Bsmat3!(Bs, ecoords_e)
        add_btdb_ut_only!(elmat, Bs, (1/3)*(t^3/(t^2+mult_el_size*he^2))*Ae, Dt, DtBs)
...
function stiffness(self::FEMMShellT3FFA, geom0::NodalField{FFlt}, u1::NodalField{TI}, Rfield1::NodalField{TI}, dchi::NodalField{TI}) where {TI<:Number}
...
"""
    mass(self::FEMMShellT3FFA,  assembler::A,  geom::NodalField{FFlt}, dchi::NodalField{TI}) where {A<:AbstractSysmatAssembler, TI<:Number}

Compute the consistent mass matrix

This is a general routine for the shell FEMM.
"""
function mass(self::FEMMShellT3FFA,  assembler::A,  geom0::NodalField{FFlt}, dchi::NodalField{TI}) where {A<:AbstractSysmatAssembler, TI<:Number}
    @assert self._associatedgeometry == true
    fes = self.integdomain.fes
    label = self.integdomain.fes.label
    normals, normal_valid = self._normals, self._normal_valid
...
function mass(self::FEMMShellT3FFA,  geom::NodalField{FFlt},  u::NodalField{TI}) where {TI<:Number}
    assembler = SysmatAssemblerSparseSymm();
...
"""
function inspectintegpoints(self::FEMMShellT3FFA, geom0::NodalField{FFlt},  u::NodalField{TI}, dT::NodalField{FFlt}, felist::FIntVec, inspector::F, idat, quantity=:Cauchy; context...) where {TI<:Number, F<:Function}
    @assert self._associatedgeometry == true
    fes = self.integdomain.fes
    label = self.integdomain.fes.label
    normals, normal_valid = self._normals, self._normal_valid
...
function inspectintegpoints(self::FEMMShellT3FFA, geom::NodalField{FFlt},  u::NodalField{TI}, felist::FIntVec, inspector::F, idat, quantity=:Cauchy; context...) where {TI<:Number, F<:Function}
...
function _resultant_check(self::FEMMShellT3FFA, geom0::NodalField{FFlt}, u1::NodalField{TI}, Rfield1::NodalField{TI}, dchi::NodalField{TI}) where {ASS<:AbstractSysmatAssembler, TI<:Number}
    @assert self._associatedgeometry == true
    fes = self.integdomain.fes
    label = self.integdomain.fes.label
    normals, normal_valid = self._normals, self._normal_valid
