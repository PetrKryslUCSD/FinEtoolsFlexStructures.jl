...
function _execute(n = 8, visualize = true)
    ...
    associategeometry!(femm, geom0)
    # @show count(fens)
    # @show formul.num_normals(femm)
    # @show femm._normals
    vtkwrite("scordelis_lo-$n-normals.vtu", fens, fes; vectors = [("normals", deepcopy(femm._normals[:, 1:3]))])
...
function _execute(n = 8, visualize = true)
    ...
    # @show formul.num_normals(femm)
    # @show femm._normals
    vtkwrite("sphere-$n-normals.vtu", fens, fes; vectors = [("normals", deepcopy(femm._normals[:, 1:3]))])
