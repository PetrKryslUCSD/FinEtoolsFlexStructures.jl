...
function test()
    ...
    for (c, r) in zip(
        (par.A, par.J, par.I1, par.I2, par.I3),
        (
            112.75829799164978,
            2023.5649824692157,
            2023.5649824692157,
...
function test()
    ri, re = 3.9910, 5.9910
    cs = CrossSectionModule.CrossSectionHollowCircle(
        s -> 3.9910,
        s -> 5.9910,
        s -> [0.0, 0.0, 1.0],
    )
...
function test()
    ...
    for (c, r) in zip(
        (par.A, par.J, par.I1, par.I2, par.I3),
        (176.4, 970.849152, 26190.108000000004, 259.30800000000005, 25930.800000000003),
    )
        @test c ≈ r
    end
    true
...
using Test
function test()
    cs = CrossSectionModule.CrossSectionRectangle(
        s -> 42.0,
        s -> 4.2,
        s -> [0.0, 0.0, 1.0],
        5 / 6,
...
function test()
    R = 0.5
    cs = CrossSectionModule.CrossSectionCircle(
        s -> (1 / 2 + 2 * s) * R,
        s -> [0.0, 0.0, 1.0],
    )
    par = cs.parameters(0.0)
